...
tracked_files:
  - AtatusSessionReplay/Sources/SessionReplay.swift
  - AtatusSessionReplay/Sources/SessionReplayConfiguration.swift
  - AtatusSessionReplay/Sources/SessionReplayPrivacyOverrides.swift
  - AtatusSessionReplay/Sources/SessionReplayPrivacyView.swift
  - AtatusInternal/Sources/Models/SessionReplay/SessionReplayConfiguration.swift
---
...
### Feature Entry Point
- **`AtatusSessionReplay/Sources/SessionReplay.swift`** - Main entry point. Call `SessionReplay.enable(with:)` to activate.
  - `startRecording()` - Start recording manually
  - `stopRecording()` - Stop recording manually

### Configuration
- **`AtatusSessionReplay/Sources/SessionReplayConfiguration.swift`** - All configuration options
  - Sampling rate, privacy levels, feature flags

### Privacy Overrides (Per-View)
- **`AtatusSessionReplay/Sources/SessionReplayPrivacyOverrides.swift`** - UIKit per-view privacy control
  - Access via `view.dd.sessionReplayPrivacyOverrides`
  - Override text, image, touch privacy per view
  - Hide specific views entirely
- **`AtatusSessionReplay/Sources/SessionReplayPrivacyView.swift`** - SwiftUI per-view privacy control (iOS 16+)
  - Use `SessionReplayPrivacyView { ... }` wrapper
  - Same privacy options as UIKit overrides

### Privacy Level Enums
- **`AtatusInternal/Sources/Models/SessionReplay/SessionReplayConfiguration.swift`** - Privacy level definitions
...
### Implementation
- **`AtatusSessionReplay/Sources/Feature/SessionReplayFeature.swift`** - Internal feature implementation
...
- **WebView Tracking**: Enables Session Replay in web views. Requires:
  ...
  - See `AtatusWebViewTracking/Sources/WebViewTracking.swift`
