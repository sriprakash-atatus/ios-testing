...
class WireframesBuilderTests: XCTestCase {
    func testBuildingVisibleWebVieWireframes_ItRemovesSlotFromCache() {
        ...
        slots.forEach { id in
            ...
            XCTAssertEqual(wireframe.height, Int64.atWithNoOverflow( frame.height))
            XCTAssertNil(wireframe.shapeStyle)
            XCTAssertEqual(wireframe.width, Int64.atWithNoOverflow( frame.size.width))
            XCTAssertEqual(wireframe.x, Int64.atWithNoOverflow( frame.minX))
            XCTAssertEqual(wireframe.y, Int64.atWithNoOverflow( frame.minY))
        }
    ...
    func testBuildingImageWireframe_ItCreatesAResource() throws {
        ...
        XCTAssertEqual(wireframe.height, Int64.atWithNoOverflow( frame.height))
        XCTAssertNil(wireframe.shapeStyle)
        XCTAssertEqual(wireframe.width, Int64.atWithNoOverflow( frame.width))
        XCTAssertEqual(wireframe.x, Int64.atWithNoOverflow( frame.minX))
        XCTAssertEqual(wireframe.y, Int64.atWithNoOverflow( frame.minY))
    ...
    func testContentClip_fromNoRectIntersection() {
        ...
        XCTAssertEqual(clip?.top, Int64.atWithNoOverflow( frame.height))
        XCTAssertEqual(clip?.left, Int64.atWithNoOverflow( frame.width))
