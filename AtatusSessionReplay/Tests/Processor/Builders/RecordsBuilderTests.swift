...
class RecordsBuilderTests: XCTestCase {
    ...
    func testWhenPreviousAndNextWireframesAreDifferent_itCreatesIncrementalSnapshotRecord() throws {
        ...
        ATAssertReflectionEqual(mutations.adds[0].wireframe, next[2])
    }
    ...
    func testWhenWireframesAreNotConsistent_itFallbacksToFullSnapshotRecordAndSendsErrorTelemetry() throws {
        ...
        ATAssertReflectionEqual(fullRecord.data.wireframes, next)
        XCTAssertEqual(
            telemetry.description,
            """
            Telemetry logs:
             - [error] [SR] Failed to create incremental record - typeMismatch(fromType: "shape", toType: "text"), kind: WireframeMutationError, stack: AtatusSessionReplay/RecordsBuilder.swift:73
            """
        )
    }
}
