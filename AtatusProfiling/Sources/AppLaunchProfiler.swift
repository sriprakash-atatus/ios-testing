...
extension ProfilingContext.Status {
    ...
    init(_ status: dd_profiler_status_t) {
        switch status {
        case AT_PROFILER_STATUS_NOT_STARTED:
            self = .stopped(reason: .notStarted)
        case AT_PROFILER_STATUS_RUNNING:
            self = .running
        case AT_PROFILER_STATUS_STOPPED:
            self = .stopped(reason: .manual)
        case AT_PROFILER_STATUS_TIMEOUT:
            self = .stopped(reason: .timeout)
        case AT_PROFILER_STATUS_PREWARMED:
            self = .stopped(reason: .prewarmed)
        case AT_PROFILER_STATUS_ALLOCATION_FAILED:
