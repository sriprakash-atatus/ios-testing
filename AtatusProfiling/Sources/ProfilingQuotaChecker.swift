...
internal struct ProfilingQuotaResult: Equatable {
    ...
    let reason: ATProfiling.QuotaReason
}
...
extension ProfilingQuotaChecker: FeatureMessageReceiver {
    ...
    private func request(sessionID: String, context: AtatusContext) -> URLRequest {
        ...
        request.setValue(context.licenseKey, forHTTPHeaderField: URLRequestBuilder.HTTPHeader.atClientTokenHeaderField)
...
private struct QuotaResponse: Decodable {
    ...
    struct Attributes: Decodable {
        ...
        private static func normalizedReason(from rawValue: String?) -> ATProfiling.QuotaReason {
            switch rawValue {
            case ATProfiling.QuotaReason.backendUnavailable.rawValue, "backend_client_not_initialized":
                return .backendUnavailable
            case let rawValue:
                return rawValue.flatMap(ATProfiling.QuotaReason.init(rawValue:)) ?? .undefined
            }
        }
    }
