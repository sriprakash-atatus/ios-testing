...
final class AppLaunchProfilerTests: XCTestCase {
    ...
    func testReceive_withTTIDMessage_whenProfilerSampledOut_returnsFalse() {
        ...
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_NOT_STARTED, "Profiler should be sampled out")
    ...
    func testReceive_withTTIDMessage_whenProfilerPrewarmed_returnsFalse() {
        ...
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_PREWARMED, "Profiler should be prewarmed")
    ...
    func testProfilingContextStatus_mapsCorrectlyFromDDProfilerStatus() {
        let cases: [(dd_profiler_status_t, ProfilingContext.Status)] = [
            (AT_PROFILER_STATUS_NOT_STARTED, .stopped(reason: .notStarted)),
            (AT_PROFILER_STATUS_RUNNING, .running),
            (AT_PROFILER_STATUS_STOPPED, .stopped(reason: .manual)),
            (AT_PROFILER_STATUS_TIMEOUT, .stopped(reason: .timeout)),
            (AT_PROFILER_STATUS_PREWARMED, .stopped(reason: .prewarmed)),
            (AT_PROFILER_STATUS_ALLOCATION_FAILED, .error(reason: .memoryAllocationFailed)),
        ]
    ...
    func testIsProfilingEnabled_whenKeyIsTrue() {
        // Given
        let userDefaults = UserDefaults(suiteName: AT_PROFILING_USER_DEFAULTS_SUITE_NAME)
    ...
    func testIsProfilingDisabled_whenKeyIsFalse() {
        // Given
        let userDefaults = UserDefaults(suiteName: AT_PROFILING_USER_DEFAULTS_SUITE_NAME)
        userDefaults?.setValue(false, forKey: AT_PROFILING_IS_ENABLED_KEY)
    ...
    func testProfilingDefaults_persistAcrossTestCases() {
        ...
        let userDefaults = UserDefaults(suiteName: AT_PROFILING_USER_DEFAULTS_SUITE_NAME)
        userDefaults?.setValue(true, forKey: AT_PROFILING_IS_ENABLED_KEY)
        let otherUserDefaults = UserDefaults(suiteName: AT_PROFILING_USER_DEFAULTS_SUITE_NAME)

        // Then
        XCTAssertTrue(otherUserDefaults?.value(forKey: AT_PROFILING_IS_ENABLED_KEY) as? Bool ?? false)
        XCTAssertTrue(dd_is_profiling_enabled())
    }
...
private extension AppLaunchProfilerTests {
    ...
    func appLaunchProfiler(
        core: AtatusCoreProtocol = PassthroughCoreMock(),
