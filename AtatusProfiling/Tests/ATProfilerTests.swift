...
final class ATProfilerTests: XCTestCase {
    ...
    func testDDProfiler_initiallyNotStarted() {
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_NOT_STARTED, "Profiler should exist but not be running until started")
    }

    func testDDProfiler_startTesting_withValidSampleRate_startsSuccessfully() {
        XCTAssertEqual(dd_profiler_start(), 1)
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_RUNNING, "Profiler should be running after starting with valid sample rate")
    }

    func testDDProfiler_startTesting_withZeroSampleRate_doesNotStart() {
        dd_profiler_start_testing(0, false, 5.seconds.dd.toInt64Nanoseconds, 0)
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_NOT_STARTED, "Profiler should not start with zero sample rate")
    }
    ...
    func testDDProfiler_startTesting_withSampleRateAbove100_startsSuccessfully() {
        ...
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_RUNNING, "Profiler should start successfully with sample rate above 100")
    }
    ...
    func testDDProfiler_startTesting_withCustomTimeout() {
        ...
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_RUNNING, "Profiler should start with custom timeout")
    }
    ...
    func testDDProfiler_startTesting_withPrewarming_doesNotStart() {
        ...
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_PREWARMED, "Profiler should not start when prewarming is active")
    }

    func testDDProfiler_stop_whenRunning_stopsSuccessfully() {
        // Given
        XCTAssertEqual(dd_profiler_start(), 1)
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_RUNNING, "Profiler should be running")

        // When
        dd_profiler_stop()

        // Then
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_STOPPED, "Profiler should be stopped after calling stop")
    }

    func testDDProfiler_stop_whenNotRunning_doesNotCrash() {
        // Given
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_NOT_STARTED, "Precondition: profiler should not be running")

        // When/Then - should not crash (`stop` is a no-op when sampling was never started)
        dd_profiler_stop()
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_NOT_STARTED, "Status should remain unchanged")
    }
    ...
    func testDDProfiler_multipleStops_doesNotCrash() {
        ...
        XCTAssertEqual(dd_profiler_start(), 1)
        dd_profiler_stop()
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_STOPPED, "Profiler should be stopped")

        // When/Then - should not crash
        dd_profiler_stop()
        dd_profiler_stop()
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_STOPPED, "Status should remain stopped")
    }
    ...
    func testDDProfiler_getProfile_whenNotStarted_returnsNil() {
        // Given
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_NOT_STARTED, "Precondition: profiler should not be started")
    ...
    func testDDProfiler_destroy_clearsAllData() {
        ...
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_NOT_CREATED, "Status should be reset to not created")
    }
    ...
    func testDDProfiler_destroy_whenNotStarted_doesNotCrash() {
        ...
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_NOT_CREATED, "Profiler Status should be not created")
    }
    ...
    func testDDProfiler_statusCodes_prewarmed() {
        dd_profiler_start_testing(100, true, 5.seconds.dd.toInt64Nanoseconds, 0) // prewarming = true
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_PREWARMED, "Should return PREWARMED status when prewarming is true")
    }
    ...
    func testConcurrentStop_doesNotCrash() {
        ...
        XCTAssertEqual(dd_profiler_get_status(), AT_PROFILER_STATUS_STOPPED)
    }
