...
final class ProfilingRUMIntegrationTests: XCTestCase {
    ...
    func testContinuousProfilingSamplesOut_customVitalDoesNotSendProfileEvent() throws {
        ...
        dd_profiler_stop()
        waitUntilProfilerStatus(AT_PROFILER_STATUS_STOPPED, description: "profiler is stopped before the custom vital")

        startVital(name: vitalName, operationKey: operationKey)
        core.flush()
        waitUntilProfilerStatus(AT_PROFILER_STATUS_STOPPED, description: "custom vital does not start the profiler")

        finishVital(name: vitalName, operationKey: operationKey)
        core.flush()
        waitUntilProfilerStatus(AT_PROFILER_STATUS_STOPPED, description: "profiler is still stopped after the custom vital")
