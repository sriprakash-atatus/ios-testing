...
class RUMSessionWithNoViewTests: RUMSessionTestsBase {
        ...
        for given in [given1, given2] {
            ...
            for when in [when1, when2, when3] {
                ...
                if session.sessionPrecondition == .prewarm {
                    ...
                    ATAssertEqual(session.timeToInitialDisplay, timeToSDKInit + timeToAppBecomeActive + dt1 + dt2 - 0.1 , accuracy: accuracy)
                } else {
    ...
    func testGivenUserSessionThatEnteredBackground_whenEventsAreTracked() throws {
        ...
        for when in [when1, when2, when3] {
            ...
            ATAssertEqual(session.duration, timeToSDKInit + timeToAppBecomeActive + timeToAppEnterBackground, accuracy: accuracy)
            XCTAssertEqual(session.sessionPrecondition, .userAppLaunch)
            XCTAssertEqual(session.views.count, 1)
            XCTAssertEqual(session.views[0].name, applicationLaunchViewName)
            ATAssertEqual(session.views[0].duration, timeToSDKInit + timeToAppBecomeActive + timeToAppEnterBackground, accuracy: accuracy)
        }
        ...
        for when in [when4, when5] {
            ...
            #endif
            ATAssertEqual(session.sessionStartDate, processLaunchDate, accuracy: accuracy)
            ATAssertEqual(session.duration, timeToSDKInit + timeToAppBecomeActive + timeToAppEnterBackground + dt1 + dt2, accuracy: accuracy)
            XCTAssertEqual(session.sessionPrecondition, .userAppLaunch)
    ...
    func testGivenUserSessionWithAutomaticViewThatEnteredBackground_whenEventsAreTracked() throws {
        ...
        for when in [when1, when2, when3] {
            ...
            ATAssertEqual(session.views[1].duration, timeToAppEnterBackground, accuracy: accuracy)
        }
        ...
        for when in [when4, when5] {
            ...
            ATAssertEqual(session.views[2].duration, dt2, accuracy: accuracy)
        }
