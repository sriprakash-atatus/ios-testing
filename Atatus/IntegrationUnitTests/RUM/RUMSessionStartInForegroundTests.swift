...
class RUMSessionStartInForegroundTests: RUMSessionTestsBase {
    ...
    func testGivenUserLaunch_whenNoEventIsTracked() throws {
        ...
        for given in givens2 {
            ...
            ATAssertEqual(session.sessionStartDate, processLaunchDate + timeToAppBecomeActive + timeToSDKInit, accuracy: accuracy)
            ATAssertEqual(session.duration, dt1, accuracy: accuracy)
            XCTAssertEqual(session.sessionPrecondition, .userAppLaunch)
            XCTAssertEqual(session.views.count, 1)
            XCTAssertEqual(session.views[0].name, applicationLaunchViewName)
            ATAssertEqual(session.views[0].duration, dt1, accuracy: accuracy)
        }
    }
    ...
    func testGivenUserLaunch_whenManualViewIsTracked() throws {
        ...
        for given in givens1 {
            ...
            for when in [when1, when2] {
                ...
                ATAssertEqual(session.views[1].duration, dt2, accuracy: accuracy)
            }
        }
        ...
        for given in givens2 {
            ...
            for when in [when1, when2] {
                ...
                XCTAssertNil(session.timeToInitialDisplay)
                ATAssertEqual(session.sessionStartDate, processLaunchDate + timeToAppBecomeActive + timeToSDKInit, accuracy: accuracy)
                ATAssertEqual(session.duration, dt1 + dt2, accuracy: accuracy)
                XCTAssertEqual(session.sessionPrecondition, .userAppLaunch)
                XCTAssertEqual(session.views.count, 2)
                XCTAssertEqual(session.views[0].name, applicationLaunchViewName)
                ATAssertEqual(session.views[0].duration, dt1, accuracy: accuracy)
    ...
    func testGivenUserLaunch_whenActionsAreTracked() throws {
        ...
        for given in givens1 {
            ...
            for when in [when1, when2] {
                ...
                XCTAssertEqual(session.views.count, 1)
                XCTAssertEqual(session.views[0].name, applicationLaunchViewName)
                ATAssertEqual(session.views[0].duration, timeToSDKInit + timeToAppBecomeActive + dt1 + dt2, accuracy: accuracy)
        ...
        for given in givens2 {
            ...
            for when in [when1, when2] {
                ...
                XCTAssertEqual(session.views.count, 1)
                XCTAssertEqual(session.views[0].name, applicationLaunchViewName)
                ATAssertEqual(session.views[0].duration, dt1 + dt2, accuracy: accuracy)
                XCTAssertNotNil(session.views[0].actionEvents.first(where: { $0.action.target?.name == "CustomAction1" }))
                XCTAssertNotNil(session.views[0].actionEvents.first(where: { $0.action.target?.name == "CustomAction2" }))
            }
        }
    ...
    func testGivenOSPrewarmLaunch_whenManualViewIsTracked() throws {
        ...
        for given in [given1, given2] {
            ...
            for when in [when1, when2] {
                ...
                let session = try when.then().takeSingle()
                XCTAssertNotNil(session.ttidEvent)
                ATAssertEqual(session.timeToInitialDisplay, preWarmedTimeToInitialDisplay, accuracy: accuracy)
                ATAssertEqual(session.sessionStartDate, processLaunchDate + timeToSDKInit + timeToAppBecomeActive + dt1, accuracy: accuracy)
