...
class RUMSessionTrackingTests: RUMSessionTestsBase {
    ...
    private func assertUserSession(
        ...
        ATAssertEqual(session.timeToInitialDisplay, timeToSDKInit + 4 + timeToAppBecomeActive, accuracy: accuracy)
        ATAssertEqual(session.sessionStartDate, processLaunchDate, accuracy: accuracy)
        ATAssertEqual(session.duration, timeToSDKInit + 4 * dt + timeToAppBecomeActive + 18 * dt, accuracy: accuracy)
        ...
        ATAssertEqual(nextView.duration, timeToSDKInit + 4 * dt + timeToAppBecomeActive, accuracy: accuracy)
        ...
        ATAssertEqual(nextView.duration, 4 * dt, accuracy: accuracy)
        ...
        ATAssertEqual(nextView.duration, 5 * dt, accuracy: accuracy)
        XCTAssertEqual(nextView.actionEvents.count, 2)
        XCTAssertEqual(nextView.resourceEvents.count, 1)
        ...
        if expectBackgroundView {
            ...
            ATAssertEqual(nextView.duration, 2 * dt, accuracy: accuracy)
            XCTAssertEqual(nextView.actionEvents.count, 0)
            XCTAssertEqual(nextView.resourceEvents.count, 1)
            XCTAssertEqual(nextView.longTaskEvents.count, 0)
        }
    ...
    private func assertBackgroundSession(
        ...
        ATAssertEqual(session.sessionStartDate, processLaunchDate + timeToSDKInit + dt, accuracy: accuracy)
        ATAssertEqual(session.duration, 5 * dt + LaunchReasonResolver.Constants.launchWindowThreshold + 6 * dt, accuracy: accuracy)
        ...
        ATAssertEqual(view.duration, 5 * dt + LaunchReasonResolver.Constants.launchWindowThreshold + 6 * dt, accuracy: accuracy)
    ...
    private func assertSuspendedBackgroundSession(
        ...
        XCTAssertNil(session.timeToInitialDisplay)
        ATAssertEqual(session.sessionStartDate, processLaunchDate + timeToSDKInit + dt, accuracy: accuracy)
        ATAssertEqual(session.duration, 3 * dt + LaunchReasonResolver.Constants.launchWindowThreshold + 2 * dt, accuracy: accuracy)
        XCTAssertEqual(session.sessionPrecondition, expectedSessionPrecondition)
        XCTAssertEqual(session.views.count, 1)

        let view = session.views[0]
        XCTAssertEqual(view.name, backgroundViewName)
        ATAssertEqual(view.duration, 3 * dt + LaunchReasonResolver.Constants.launchWindowThreshold + 2 * dt, accuracy: accuracy)
    ...
    private func assertResumedUserSession(
        ...
        XCTAssertNil(session.ttidEvent)
        XCTAssertNil(session.timeToInitialDisplay)
        ATAssertEqual(session.sessionStartDate, processLaunchDate + timeToSDKInit + 6 * dt + LaunchReasonResolver.Constants.launchWindowThreshold + 2.hours, accuracy: accuracy)
        ATAssertEqual(session.duration, 4 * dt, accuracy: accuracy)
        XCTAssertEqual(session.sessionPrecondition, expectedSessionPrecondition)
        XCTAssertEqual(session.views.count, 1)

        let view = session.views[0]
        XCTAssertEqual(view.name, "FirstView")
        ATAssertEqual(view.duration, 4 * dt, accuracy: accuracy)
