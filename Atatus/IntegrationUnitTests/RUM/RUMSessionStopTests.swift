...
class RUMSessionStopTests: RUMSessionTestsBase {
    ...
    func testGivenUserSession_whenItIsStopped_andActionIsTrackedInForeground() throws {
        ...
        for given in [given1, given2, given3, given4, given5, given6] {
            ...
            ATAssertEqual(session1.duration, timeToSDKInit + timeToAppBecomeActive + dt1, accuracy: accuracy)
            ...
            if given == given1 || given == given2 { // session with `ApplicationLaunch` view
                ...
                ATAssertEqual(session1.views[0].duration, timeToSDKInit + timeToAppBecomeActive + dt1, accuracy: accuracy)
                ...
                ATAssertEqual(session1.views[1].duration, dt1, accuracy: accuracy)
            }
            ...
            ATAssertEqual(session2.sessionStartDate, processLaunchDate + timeToSDKInit + timeToAppBecomeActive + dt1 + dt2, accuracy: accuracy)
        ...
        for given in [given1, given2, given3] {
            ...
            for when in [
                ...
                if given == given1 { // session with `ApplicationLaunch` view
                    ...
                    ATAssertEqual(session.duration, timeToSDKInit + timeToAppBecomeActive + dt1 + dt2, accuracy: accuracy)
                    XCTAssertEqual(session.views.count, 2)
                    XCTAssertEqual(session.views[0].name, applicationLaunchViewName)
                    ATAssertEqual(session.views[0].duration, timeToSDKInit + timeToAppBecomeActive, accuracy: accuracy)
                    XCTAssertEqual(session.views[1].name, manualViewName)
                    ATAssertEqual(session.views[1].duration, dt1 + dt2, accuracy: accuracy)
                }
            }
        }
    }
    ...
    func testGivenUserSession_andBETEnabled_whenItIsStopped_andActionIsTrackedInBackground() throws {
        ...
        for given in [given1, given2, given3] {
            ...
            for when in [when1, when2] {
                ...
                XCTAssertNil(session2.ttidEvent)
                XCTAssertNil(session2.timeToInitialDisplay)
                ATAssertEqual(session2.sessionStartDate, processLaunchDate + timeToSDKInit + timeToAppBecomeActive + dt1 + dt2 + dt3, accuracy: accuracy)
    ...
    func testGivenBackgroundSession_andBETEnabled_whenItIsStopped_andActionIsTrackedInBackground() throws {
        ...
        for given in [given1, given2] {
            ...
            ATAssertEqual(session1.duration, dt2 + dt3, accuracy: accuracy)
            ...
            ATAssertEqual(session1.views[0].duration, dt2 + dt3, accuracy: accuracy)
            XCTAssertEqual(session1.views[0].resourceEvents.count, 1)

            // - It creates new session for tracking background events:
            XCTAssertNil(session2.ttidEvent)
            XCTAssertNil(session2.timeToInitialDisplay)
            ATAssertEqual(session2.sessionStartDate, processLaunchDate + timeToSDKInit + dt1 + dt2 + dt3 + dt4, accuracy: accuracy)
    ...
    func testGivenBackgroundSession_andBETEnabled_whenItIsStopped_andOtherEventsAreTrackedInBackground() throws {
        ...
        for given in [given1, given2] {
            ...
            for when in [when1, when2] {
                ...
                ATAssertEqual(session.duration, dt2 + dt3, accuracy: accuracy)
                ...
                ATAssertEqual(session.views[0].duration, dt2 + dt3, accuracy: accuracy)
        ...
        for given in [given1, given2] {
            ...
            for when in [when1, when2] {
                ...
                for when in [when1, when2] {
                    ...
                    XCTAssertNil(session.ttidEvent)
                    XCTAssertNil(session.timeToInitialDisplay)
                    ATAssertEqual(session.sessionStartDate, processLaunchDate + timeToSDKInit + dt1 + dt2 + dt3 + dt4 + dt5, accuracy: accuracy)
                    ATAssertEqual(session.duration, dt6, accuracy: accuracy)
                    ...
                    ATAssertEqual(session.views[0].duration, dt6, accuracy: accuracy)
                }
            }
        }
        #endif
    ...
    func testGivenBackgroundSession_whenItIsStopped_andManualViewIsTrackedInForeground() throws {
        ...
        ATAssertEqual(session1when1.duration, timeToSDKInit + dt1 + dt2 + dt3, accuracy: accuracy)
        ...
        ATAssertEqual(session1when1.views[0].duration, timeToSDKInit + dt1 + dt2 + dt3, accuracy: accuracy)
        ...
        ATAssertEqual(session2when1.sessionStartDate, processLaunchDate + timeToSDKInit + dt1 + dt2 + dt3 + dt4 + dt5, accuracy: accuracy)
        ATAssertEqual(session2when1.duration, dt6, accuracy: accuracy)
        ...
        ATAssertEqual(session2when1.views[0].duration, dt6, accuracy: accuracy)
        ...
        ATAssertEqual(session1when2.duration, timeToSDKInit + dt1 + dt2 + dt3 + dt4, accuracy: accuracy)
        ...
        ATAssertEqual(session1when2.views[0].duration, timeToSDKInit + dt1 + dt2 + dt3 + dt4, accuracy: accuracy)
        ...
        ATAssertEqual(session2when2.sessionStartDate, processLaunchDate + timeToSDKInit + dt1 + dt2 + dt3 + dt4 + dt5, accuracy: accuracy)
        ATAssertEqual(session2when2.duration, dt6, accuracy: accuracy)
        ...
        ATAssertEqual(session2when2.views[0].duration, dt6, accuracy: accuracy)
    }
    #endif
    ...
    func testGivenBackgroundSession_andBETEnabled_whenItIsStopped_andViewIsTrackedInForeground() throws {
        ...
        for given in [given1, given2] {
            ...
            for when in [when1, when2] {
                ...
                XCTAssertNil(session2.ttidEvent)
                XCTAssertNil(session2.timeToInitialDisplay)
                ATAssertEqual(session2.sessionStartDate, processLaunchDate + timeToSDKInit + dt1 + dt2 + dt3 + dt4 + dt5, accuracy: accuracy)
                ATAssertEqual(session2.duration, dt6, accuracy: accuracy)
                XCTAssertEqual(session2.sessionPrecondition, .explicitStop)
                XCTAssertEqual(session2.views.count, 1)
                XCTAssertEqual(session2.views[0].name, when == when1 ? automaticViewName : manualViewName)
                ATAssertEqual(session2.views[0].duration, dt6, accuracy: accuracy)
            }
    ...
    func testGivenBackgroundSession_andBETEnabled_whenItIsStopped_andManualViewIsTrackedInForeground() throws {
        ...
        ATAssertEqual(session1a.duration, timeToSDKInit + dt1 + dt2 + dt3, accuracy: accuracy)
        ...
        ATAssertEqual(session1a.views[0].duration, timeToSDKInit + dt1 + dt2 + dt3, accuracy: accuracy)
        ...
        ATAssertEqual(session2a.sessionStartDate, processLaunchDate + timeToSDKInit + dt1 + dt2 + dt3 + dt4 + dt5, accuracy: accuracy)
        ATAssertEqual(session2a.duration, dt6, accuracy: accuracy)
        ...
        ATAssertEqual(session2a.views[0].duration, dt6, accuracy: accuracy)
        ...
        ATAssertEqual(session1b.duration, timeToSDKInit + dt1 + dt2 + dt3 + dt4, accuracy: accuracy)
        ...
        ATAssertEqual(session1b.views[0].duration, timeToSDKInit + dt1 + dt2 + dt3 + dt4, accuracy: accuracy)
        ...
        ATAssertEqual(session2b.sessionStartDate, processLaunchDate + timeToSDKInit + dt1 + dt2 + dt3 + dt4 + dt5, accuracy: accuracy)
        ATAssertEqual(session2b.duration, dt6, accuracy: accuracy)
        ...
        ATAssertEqual(session2b.views[0].duration, dt6, accuracy: accuracy)
    }
    #endif
}
