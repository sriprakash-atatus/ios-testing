...
public enum WebViewTracking {
    ...
    public static func disable(webView: WKWebView) {
        runOnMainThreadSync {
            ...
            controller.removeScriptMessageHandler(forName: ATScriptMessageHandler.name)
    ...
    private static func prepareWebView(
        ...
        guard !isTracking else {
            AT.logger.warn("`\(callerName)` was called more than once for the same WebView. Second call will be ignored. Make sure you call it only once.")
            return false
        }

        let bridgeName = ATScriptMessageHandler.name
        let messageHandler = ATScriptMessageHandler(
    ...
    static func update(_ webView: WKWebView, in core: AtatusCoreProtocol, using elements: WebViewTrackingElements, isTraceSampled: String) -> Bool {
        ...
        (function updateBridge(win) {
            try {
                if (win.\(ATScriptMessageHandler.name)) {
                    win.\(ATScriptMessageHandler.name).getIsTraceSampled = () => '\(isTraceSampled)';
                }
