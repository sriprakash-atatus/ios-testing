...
class AtatusCrashReportFilterTests: XCTestCase {
    ...
    private func parseStackFrame(_ frame: String) throws -> (index: Int, libraryName: String, instructionAddr: String, loadAddr: String, offset: Int) {
        ...
        guard
            ...
            throw ATAssertError.expectedFailure("Stack trace of unexpected format")
        }
    ...
    func testFilterReports_ConvertsValidCrashReportToDDCrashReport() throws {
        ...
        let atReport = try XCTUnwrap(capturedReports?.first?.untypedValue as? ATCrashReport)

        // Verify error type and message
        XCTAssertEqual(atReport.type, "SIGSEGV (SEGV_MAPERR)", "Should format type as 'signal (code)'")
        XCTAssertEqual(atReport.message, "Application crash: SIGSEGV (Segmentation fault)", "Should include signal description")

        // Verify metadata
        XCTAssertEqual(atReport.meta.incidentIdentifier, "incident-123", "Should extract incident identifier")
        XCTAssertEqual(atReport.meta.process, "MyApp [12345]", "Should format process as 'name [pid]'")
        XCTAssertEqual(atReport.meta.parentProcess, "launchd [1]", "Should format parent process as 'name [pid]'")
        XCTAssertEqual(atReport.meta.path, "/var/containers/Bundle/Application/MyApp", "Should extract executable path")
        XCTAssertEqual(atReport.meta.codeType, "arm64", "Should extract architecture")
        XCTAssertEqual(atReport.meta.exceptionType, "SIGSEGV", "Should extract exception type")
        XCTAssertEqual(atReport.meta.exceptionCodes, "SEGV_MAPERR", "Should extract exception codes")

        // Verify threads
        XCTAssertEqual(atReport.threads.count, 1, "Should have 1 thread")
        XCTAssertTrue(atReport.threads[0].crashed, "Thread should be marked as crashed")
        XCTAssertEqual(atReport.threads[0].name, "Thread 0", "Should default thread name")
        XCTAssertFalse(atReport.threads[0].stack.isEmpty, "Thread should have stack")

        // Verify stack format using regex parser
        let stackLines = atReport.stack.split(separator: "\n")
        ...
        XCTAssertEqual(atReport.binaryImages.count, 1, "Should have 1 binary image")
        let binaryImage = atReport.binaryImages[0]
        ...
        // Verify context
        XCTAssertEqual(atReport.context, Data(#"{"dd":"🐶"}"#.utf8), "Should decode context data")
        XCTAssertFalse(atReport.wasTruncated, "Should not be truncated by default")
    }
    ...
    func testFilterReports_HandlesMinimalCrashReport() throws {
        ...
        // Then
        let atReport = try XCTUnwrap(capturedReports?.first?.untypedValue as? ATCrashReport)
        XCTAssertEqual(atReport.type, "<unknown> (#0)")
        XCTAssertEqual(atReport.message, "No crash reason provided")
        XCTAssertEqual(atReport.stack, "???")
    }
    ...
    func testFilterReports_ExtractsCrashedThreadStack() throws {
        ...
        XCTAssertTrue(atReport.stack.contains("MyFramework"), "Stack should contain crashed thread frames")
        XCTAssertFalse(atReport.stack.contains("libsystem"), "Stack should not contain non-crashed thread frames")
        ...
        // Verify threads
        XCTAssertEqual(atReport.threads.count, 2, "Should have 2 threads")
        XCTAssertFalse(atReport.threads[0].crashed, "First thread should not be crashed")
        XCTAssertTrue(atReport.threads[1].crashed, "Second thread should be crashed")
        XCTAssertEqual(atReport.threads[1].stack, atReport.stack, "Crashed thread stack should match main stack")
    }
    ...
    func testFilterReports_DetectsSystemVsUserBinaryImages() throws {
        ...
        // Then
        let atReport = try XCTUnwrap(capturedReports?.first?.untypedValue as? ATCrashReport)
        XCTAssertEqual(atReport.binaryImages.count, 2, "Should have 2 binary images")

        // Verify system image details
        let systemImage = try XCTUnwrap(atReport.binaryImages.first { $0.libraryName == "Foundation" })
        ...
        let userImage = try XCTUnwrap(atReport.binaryImages.first { $0.libraryName == "MyApp" })
    ...
    func testFilterReports_MarksTruncatedBacktraces() throws {
        ...
        // Then
        let atReport = try XCTUnwrap(capturedReports?.first?.untypedValue as? ATCrashReport)
        XCTAssertTrue(atReport.wasTruncated)
    }
    ...
    func testFilterReports_HandlesMultipleReports() throws {
        ...
        let atReport1 = try XCTUnwrap(capturedReports?[0].untypedValue as? ATCrashReport)
        let atReport2 = try XCTUnwrap(capturedReports?[1].untypedValue as? ATCrashReport)
        XCTAssertEqual(atReport1.meta.incidentIdentifier, "1")
        XCTAssertEqual(atReport2.meta.incidentIdentifier, "2")
    }
