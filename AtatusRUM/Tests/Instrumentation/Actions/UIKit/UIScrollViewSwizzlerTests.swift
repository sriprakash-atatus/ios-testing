...
class UIScrollViewSwizzlerTests: XCTestCase {
    ...
    func testSwizzle_whenThirdPartyProxyCapturesAtatusProxy_doesNotCreateCircularRespondsToRecursion() throws {
        ...
        // `atProxy` back and store it as its forward target, creating a circular chain:
        //   atProxy.originalDelegate = thirdPartyProxy, thirdPartyProxy.forwardToDelegate = atProxy
        // Any call to `responds(to:)` on `atProxy` would then infinitely recurse → stack overflow.
        //
        // With the getter swizzle, the getter transparently returns the app's delegate (not `atProxy`),
        // so the third-party proxy stores the real delegate and the chain stays linear:
        //   atProxy → thirdPartyProxy → originalDelegate (no cycle)
        ...
        // App sets delegate — swizzler wraps it in atProxy; getter transparently returns the app's delegate
        scrollView.delegate = originalDelegate
        XCTAssertTrue(scrollView.delegate === originalDelegate)

        // Third-party proxy reads the delegate (gets app's delegate, not atProxy) and sets itself
        ...
        scrollView.delegate = thirdPartyProxy  // triggers swizzler → atProxy.originalDelegate = thirdPartyProxy
