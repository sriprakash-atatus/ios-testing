...
class AppHangsMonitorTests: XCTestCase {
    ...
    private var dd: ATMock<CoreLoggerMock>! // swiftlint:disable:this implicitly_unwrapped_optional
    private var monitor: AppHangsMonitor! // swiftlint:disable:this implicitly_unwrapped_optional

    override func setUp() {
        dd = AT.mockWith(logger: CoreLoggerMock())
    ...
    func testWhenAppHangEnds_itSendsAppHangCommand() throws {
        ...
        ATAssertReflectionEqual(command.threads, hang.backtraceResult.threads)
        ATAssertReflectionEqual(command.binaryImages, hang.backtraceResult.binaryImages)
    ...
    func testWhenSendingRUMViewEvent_itIsLinkedToPreviousRUMSessionAndIncludesErrorInformation() throws {
        ...
        ATAssertJSONDiff(lastView, viewEvent) { diffs in
            diffs.assertExact(
                different: [
        ...
        ATAssertReflectionEqual(viewEvent.device, lastView.device)
        ATAssertReflectionEqual(viewEvent.os, lastView.os)
        ATAssertJSONEqual(viewEvent.connectivity, lastView.connectivity)
        ATAssertJSONEqual(viewEvent.usr, lastView.usr)
    }
    ...
    func testWhenSendingRUMErrorEvent_itIsLinkedToPreviousRUMSessionAndIncludesErrorInformation() throws {
        ...
        ATTAssertValidRUMUUID(errorEvent.error.id)
        ...
        ATAssertJSONEqual(errorEvent.error.threads, hangBacktrace.threads.toRUMDataFormat)
        ATAssertJSONEqual(errorEvent.error.binaryImages, hangBacktrace.binaryImages.toRUMDataFormat)
