...
internal struct CrashReportReceiver: FeatureMessageReceiver {
    ...
    private func send(report: ATCrashReport, with context: CrashContext) -> Bool {
        ...
        if var lastRUMViewEvent = context.lastRUMViewEvent {
            ...
            if lastRUMViewEvent.view.crash?.count ?? 0 < 1 {
                ...
                AT.logger.debug("There was a crash in previous session, but it is ignored due to another crash already present in the last view.")
                return false
            }
    ...
    private func sendCrashReportLinkedToLastViewInPreviousSession(
        ...
        if crashTimings.realDateNow.timeIntervalSince(crashTimings.realCrashDate) < FatalErrorBuilder.Constants.viewEventAvailabilityThreshold {
            ...
            AT.logger.debug("Sending crash as RUM error.")
            featureScope.eventWriteContext(bypassConsent: true) { context, writer in
                ...
                if let mappedError = self.eventsMapper.map(event: rumError) {
                    writer.write(value: mappedError.withAgentInfo())
                } else {
                    AT.logger.warn("errorEventMapper returned 'nil' for a crash. Discarding crashes is not supported. The unmodified event will be sent.")
                    writer.write(value: rumError.withAgentInfo())
                }
            }
        }
    }
    ...
    private func sendCrashReportToPreviousSession(
        ...
        case .doNotHandle:
            AT.logger.debug("There was a crash in background, but it is ignored due to Background Event Tracking disabled or sampling.")
            newRUMView = nil
        }
    ...
    private func sendCrashReportToNewSession(
        ...
        guard sampled else {
            AT.logger.debug("There was a crash in previous session, but it is ignored due to sampling.")
            return false
        }
        ...
        case .doNotHandle:
            AT.logger.debug("There was a crash in background, but it is ignored due to Background Event Tracking disabled.")
            newRUMView = nil
        }
    ...
    private func send(crashReport: ATCrashReport, to rumView: RUMViewEvent, using crashTimings: AdjustedCrashTimings) {
        AT.logger.debug("Updating RUM view with crash report.")
        ...
        featureScope.eventWriteContext(bypassConsent: true) { context, writer in
            ...
            // ATCHG: Bind the mapped view before appending the `agent` object. The event is only
            // written when the mapper returns a value — previously a `nil` result was written as a
            // JSON `null`, which cannot carry the `agent` object.
            if let mappedView = self.eventsMapper.map(event: updatedRUMView),
               let remappedView = self.eventsMapper.map(event: mappedView) {
                writer.write(value: remappedView.withAgentInfo())
            }
            // ATCHG: End
        }
    }
