...
private extension LongTaskObserver {
    ...
    convenience init?(threshold: TimeInterval?, dateProvider: DateProvider) {
        ...
        guard threshold > RUMInstrumentation.Constants.minLongTaskThreshold else {
            AT.logger.error("`RUM.Configuration.longTaskThreshold` cannot be less than 0s. Long Tasks monitoring will be disabled.")
            return nil
        }
...
private extension AppHangsMonitor {
    ...
    convenience init?(
        ...
        if appHangThreshold < RUMInstrumentation.Constants.minAppHangThreshold {
            ...
            AT.logger.warn("`RUM.Configuration.appHangThreshold` cannot be less than \(RUMInstrumentation.Constants.minAppHangThreshold)s. A value of \(RUMInstrumentation.Constants.minAppHangThreshold)s will be used.")
        }
