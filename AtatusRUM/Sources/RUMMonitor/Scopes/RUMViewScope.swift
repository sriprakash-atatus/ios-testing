...
extension RUMViewScope {
    ...
    private func addViewLoadingTime(on command: RUMAddViewLoadingTime) {
        if viewLoadingTime == nil {
            ...
            needsViewUpdate = true
            AT.logger.debug("View loading time \(time)ns added to the view \(viewName)")
            ...
            viewLoadingTime = time
            needsViewUpdate = true
            AT.logger.warn("View loading time already exists for the view \(viewName). Replacing the existing \(String(describing: viewLoadingTime))ns with the new \(time)ns loading time.")
    ...
    private func sendViewUpdateEvent(on command: RUMCommand, context: AtatusContext, writer: Writer) {
        ...
        if interactionToNextViewTime == .failure(.disabled),
           ...
           let customInvValue = Int64(exactly: customInvValue) {
            interactionToNextViewTime = .success(TimeInterval.atFromNanoseconds( customInvValue))
        }
