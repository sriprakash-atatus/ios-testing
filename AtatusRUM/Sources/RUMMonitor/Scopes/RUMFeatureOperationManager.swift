...
internal class RUMFeatureOperationManager {
    ...
    func process(_ command: RUMOperationStepVitalCommand, context: AtatusContext, writer: Writer, activeView: RUMViewScope?) {
        ...
        if activeView == nil {
            AT.logger.warn("RUM operation step command received without an active view. This may result in incomplete context information.")
        }
    ...
    private func trackOperationStart(name: String, operationKey: String?, lookupKey: String) {
        ...
        if activeOperations.contains(lookupKey) {
            ...
            AT.logger.warn("Operation \(formatOperationName(name, operationKey: operationKey)) has already been started. This may result in the backend terminating the previous instance with an `auto_restart` failure. Note that the SDK only tracks operations locally and not across sessions.")
        }
    ...
    private func trackOperationUpdate(name: String, operationKey: String?, lookupKey: String, stepType: RUMVitalOperationStepEvent.Vital.StepType) {
        ...
        if !activeOperations.contains(lookupKey) {
            ...
            AT.logger.warn("`\(stepType.rawValue)` was called, but operation \(formatOperationName(name, operationKey: operationKey)) is currently not active. This may lead to a backend `instrumentation_error`. Make sure to call `startOperation(name:operationKey:attributes:options:)` first. Note that the SDK only tracks operations locally and not across sessions.")
        }
    ...
    private func validateName(_ value: String, stepType: RUMVitalOperationStepEvent.Vital.StepType) -> Bool {
        ...
        guard !trimmed.isEmpty else {
            AT.logger.error("Operation `name` cannot be empty or contain only whitespace/line breaks. \(stepType) command will be ignored.")
            return false
        }

        if !value.unicodeScalars.allSatisfy(Self.validOperationNameCharacters.contains) {
            AT.logger.warn("Operation `name` '\(value)' does not match the backend-accepted pattern [\\w.@$-]* (letters, digits, _ . @ $ -). The \(stepType) command will still be emitted and may be rejected by the backend.")
        }
    ...
    private func validateOperationKey(_ value: String, stepType: RUMVitalOperationStepEvent.Vital.StepType) -> Bool {
        ...
        if trimmed.isEmpty {
            AT.logger.warn("Operation `operationKey`, when provided, cannot be empty or contain only whitespace/line breaks. The \(stepType) command will still be emitted.")
        }
