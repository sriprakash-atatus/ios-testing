...
class NetworkInstrumentationFeatureTests: XCTestCase {
    ...
    func testAutomaticMode_doesNotTrackSDKRequests() throws {
        ...
        // Track if any requests with api-key are intercepted
        var interceptedSDKRequests: [URLSessionTaskInterception] = []
        handler.onInterceptionDidStart = { interception in
            interceptedSDKRequests.append(interception)
        }

        // When - Make a request to a custom endpoint with api-key header (simulating SDK internal request)
        ...
        // Then - Verify SDK request with api-key was not intercepted
        XCTAssertEqual(interceptedSDKRequests.count, 0, "Should not intercept SDK requests with api-key header, even to custom endpoints")
    }
    ...
    /// the test process (e.g. `ddSDKTesting`) was reaching the test handler and over-fulfilling
