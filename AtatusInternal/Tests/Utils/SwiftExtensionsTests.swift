...
class TimeIntervalExtensionTests: XCTestCase {
    func testTimeIntervalFromMilliseconds() {
        ...
        let timeInterval = TimeInterval.atFromMilliseconds(milliseconds)
...
class IntegerOverflowExtensionTests: XCTestCase {
    func testHappyPath() {
        ...
        XCTAssertNoThrow(try UInt64.atWithReportingOverflow(reasonableDouble))
        XCTAssertEqual(try UInt64.atWithReportingOverflow(reasonableDouble), 1_000)
    }

    func testNegative() {
        let negativeDouble = Double(-1_000.123456)

        XCTAssertThrowsError(try UInt64.atWithReportingOverflow(negativeDouble)) { error in
    ...
    func testFloat() {
        ...
        XCTAssertNoThrow(try UInt8.atWithReportingOverflow(simpleFloat))
        XCTAssertEqual(try UInt8.atWithReportingOverflow(simpleFloat), 222)
    }

    func testGreatestFiniteMagnitude() {
        let almostInfinity = Double.greatestFiniteMagnitude

        XCTAssertThrowsError(try UInt64.atWithReportingOverflow(almostInfinity)) { error in
            XCTAssertTrue(error is FixedWidthIntegerError<Double>)
        }
    }
    ...
    func testInfinity() {
        ...
        XCTAssertThrowsError(try UInt64.atWithReportingOverflow(infinityAndBeyond)) { error in
            XCTAssertTrue(error is FixedWidthIntegerError<Double>)
        }
    }
    ...
    func testCornerCase() {
        ...
        XCTAssertThrowsError(try UInt64.atWithReportingOverflow(uInt64Max)) { error in
            XCTAssertTrue(error is FixedWidthIntegerError<Double>)
            if case let FixedWidthIntegerError.overflow(overflowingValue) = (error as! FixedWidthIntegerError<Double>) {
