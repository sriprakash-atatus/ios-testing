...
internal final class RemoteLogger: LoggerProtocol, Sendable {
    ...
    func log(level: LogLevel, message: String, error: Error?, attributes: [String: Encodable]?) {
        internalLog(level: level, message: message, error: error.map { ATError(error: $0) }, attributes: attributes)
    }

    func internalLog(level: LogLevel, message: String, error: ATError?, attributes: [String: Encodable]?, completionHandler: @escaping CompletionHandler = NOPCompletionHandler) {
...
extension RemoteLogger: InternalLoggerProtocol {
    func log(
        ...
        if errorKind != nil || errorMessage != nil || stackTrace != nil {
            ...
            atError = ATError(type: errorKind ?? "", message: errorMessage ?? "", stack: stackTrace ?? "", sourceType: sourceType ?? "ios")
        }

        internalLog(level: level, message: message, error: atError, attributes: logAttributes)
    }
    ...
    func critical(
        ...
        internalLog(
            level: .critical,
            message: message,
            error: error.map { ATError(error: $0) },
