...
internal final class ConsoleLogger: LoggerProtocol {
    ...
    func log(level: LogLevel, message: String, error: Error?, attributes: [String: Encodable]?) {
        ...
        if let error = error {
            let atError = ATError(error: error)
            errorString = buildErrorString(error: atError)
        }
    ...
    private func buildErrorString(error: ATError) -> String {
        return """
        → type: \(error.type)
...
extension ConsoleLogger: InternalLoggerProtocol {
    func log(
        ...
        if errorKind != nil || errorMessage != nil || stackTrace != nil {
            ...
            let atError = ATError(type: errorKind ?? "", message: errorMessage ?? "", stack: stackTrace ?? "")
            errorString = buildErrorString(error: atError)
        }
