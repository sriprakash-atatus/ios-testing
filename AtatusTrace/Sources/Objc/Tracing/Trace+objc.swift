...
@objc(ATTraceFirstPartyHostsTracing)
@objcMembers
@_spi(objc)
...
@objc(ATTraceURLSessionTracking)
@objcMembers
@_spi(objc)
        ...
        if let objcWriter = carrier as? objc_HTTPHeadersWriter, format == OT.formatTextMap {
            ...
            let error = NSError(
                ...
                userInfo: [
                    ...
                    NSLocalizedRecoverySuggestionErrorKey: "Use `ATHTTPHeadersWriter` carrier with `OT.formatTextMap` format."
                ]
            )
            throw error
        }
