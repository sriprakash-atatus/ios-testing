...
internal struct TracingURLSessionHandler: AtatusURLSessionHandler {
    ...
    func modify(request: URLRequest, headerTypes: Set<TracingHeaderType>, networkContext: NetworkContext?) -> (URLRequest, TraceContext?, URLSessionHandlerCapturedState?) {
        ...
        let newSpanElements = makeElementsForNewSpanContext(tracer: tracer, parentSpanContext: tracer.activeSpan?.context as? ATSpanContext, networkContext: networkContext)
    ...
    func interceptionDidComplete(interception: AtatusInternal.URLSessionTaskInterception) {
        ...
        if let trace = interception.trace {
            ...
            let newSpanElements = makeElementsForNewSpanContext(tracer: tracer, parentSpanContext: interception.activeSpanContext as? ATSpanContext)
