...
tracked_files:
  - AtatusTrace/Sources/Trace.swift
  - AtatusTrace/Sources/TraceConfiguration.swift
  - AtatusTrace/Sources/Tracer.swift
  - AtatusTrace/Sources/OpenTracing/OTTracer.swift
  - AtatusTrace/Sources/OpenTracing/OTSpan.swift
  - AtatusTrace/Sources/OpenTracing/OTFormat.swift
  - AtatusTrace/Sources/OpenTracing/OTTagValue.swift
  - AtatusTrace/Sources/OpenTelemetry/OTelTracerProvider.swift
  - AtatusTrace/Sources/Objc/Tracing/Trace+objc.swift
  - AtatusTrace/Sources/Objc/OpenTracing/OTTracer+objc.swift
  - AtatusTrace/Sources/Objc/OpenTracing/OTSpan+objc.swift
  - AtatusTrace/Sources/Objc/OpenTracing/OTSpanContext+objc.swift
  - AtatusTrace/Sources/Objc/Tracing/ATSpan+objc.swift
  - AtatusTrace/Sources/Objc/Tracing/ATSpanContext+objc.swift
  - AtatusTrace/Sources/Objc/Tracing/Propagation/HTTPHeadersWriter+objc.swift
  - AtatusTrace/Sources/Objc/Tracing/Propagation/W3CHTTPHeadersWriter+objc.swift
  - AtatusTrace/Sources/Objc/Tracing/Propagation/B3HTTPHeadersWriter+objc.swift
  - AtatusTrace/Sources/Objc/Tracing/Propagation/TraceContextInjection+objc.swift
  - AtatusInternal/Sources/NetworkInstrumentation/TracingHeaderType+objc.swift
  - AtatusTrace/Sources/Feature/TraceFeature.swift
  - AtatusTrace/Sources/Integrations/TracingURLSessionHandler.swift
---
...
### Feature Entry Point
- **`AtatusTrace/Sources/Trace.swift`** — Main entry point. Call `Trace.enable(with:in:)` to activate the feature.

### Configuration
- **`AtatusTrace/Sources/TraceConfiguration.swift`** — All configuration options available to customers.
...
- **`AtatusTrace/Sources/Tracer.swift`** — Access point: `Tracer.shared(in:)` returns an `OTTracer`. Also defines `SpanTags` (`resource`, `operation`, `service`, `manualKeep`, `manualDrop`).
- **`AtatusTrace/Sources/OpenTracing/OTTracer.swift`** — OpenTracing tracer protocol (`Sendable`): `startSpan`, `startRootSpan`, `inject`, `extract`, `activeSpan`. Tag parameters use `[String: OTTagValue]?`.
- **`AtatusTrace/Sources/OpenTracing/OTSpan.swift`** — OpenTracing span protocol (`Sendable`): `setTag(key:value:)` (value is `OTTagValue`), `log(fields:)` (fields are `[String: Encodable & Sendable]`), `setBaggageItem`, `setActive`, `finish`, `setError`, `keepTrace`, `dropTrace`.
- **`AtatusTrace/Sources/OpenTracing/OTTagValue.swift`** — `public typealias OTTagValue = Encodable & Sendable`. Used for all span tag values; custom tag types must conform to both `Encodable` and `Sendable`.
- **`AtatusTrace/Sources/OpenTracing/OTFormat.swift`** — Format/carrier protocols for `inject` / `extract`.

### Public API — OpenTelemetry
- **`AtatusTrace/Sources/OpenTelemetry/OTelTracerProvider.swift`** — `OTelTracerProvider` to register with `OpenTelemetry.registerTracerProvider(...)` and use the standard OpenTelemetry `Tracer` / `SpanBuilder` API.

### Public API — Objective-C Bridge
- **`AtatusTrace/Sources/Objc/Tracing/Trace+objc.swift`** — Objective-C Trace entry point and configuration bridge (`ATTrace`, `ATTraceConfiguration`, `ATTraceURLSessionTracking`, `ATTracer`).
...
- **`AtatusTrace/Sources/Objc/OpenTracing/OTTracer+objc.swift`**, **`OTSpan+objc.swift`**, **`OTSpanContext+objc.swift`** — Objective-C OpenTracing protocols and constants.
- **`AtatusTrace/Sources/Objc/Tracing/ATSpan+objc.swift`**, **`ATSpanContext+objc.swift`** — Objective-C wrappers around Atatus span and span context implementations.
- **`AtatusTrace/Sources/Objc/Tracing/Propagation/*+objc.swift`** — Objective-C wrappers for Atatus, W3C, B3 header writers and trace context injection.
- **`AtatusInternal/Sources/NetworkInstrumentation/TracingHeaderType+objc.swift`** — Objective-C tracing header type constants used by Trace URLSession configuration.
...
### Implementation
- **`AtatusTrace/Sources/Feature/TraceFeature.swift`** — Internal feature implementation. Shows how configuration translates to behavior.
...
- **WebView Tracking**: Independent — see `AtatusWebViewTracking/Sources/WebViewTracking.swift`.
