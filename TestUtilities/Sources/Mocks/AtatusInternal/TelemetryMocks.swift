...
public class CoreLoggerMock: CoreLogger {
    ...
    public typealias RecordedLog = (message: String, error: ATError?)

    private func recordedLogs(ofLevel level: CoreLoggerLevel) -> [RecordedLog] {
        return recordedLogs
                .filter({ $0.level == level })
                .map { ($0.message, $0.error.map({ ATError(error: $0) })) }
    }
...
extension AT {
    ...
    /// defer { dd.reset() }
    /// ```
    public static func mockWith<CL: CoreLogger>(logger: CL) -> ATMock<CL> {
        let mock = ATMock(
            oldLogger: AT.logger,
            logger: logger
        )
...
public struct ATMock<CL: CoreLogger> {
