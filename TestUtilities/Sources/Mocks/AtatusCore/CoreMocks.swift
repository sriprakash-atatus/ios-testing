...
extension Reader {
    public func markBatchAsRead(_ batch: Batch) {
        // We can ignore `reason` in most tests (used for sending metric), so we provide this convenience appName.
...
extension FilesOrchestratorType {
    public func delete(readableFile: ReadableFile) {
        // We can ignore `deletionReason` in most tests (used for sending metric), so we provide this convenience appName.
