    ...
    init(
        ...
        let readWorkItem = DispatchWorkItem { [weak self] in
            ...
            if let files = files, !files.isEmpty {
                AT.logger.debug("⏳ (\(self.featureName)) Uploading batches...")
                ...
                AT.logger.debug("💡 (\(self.featureName)) No upload. Batch to upload: \(batchLabel), System conditions: \(blockersForUpload.description)")
                self.delay.increase()
            ...
            if let batch = self.fileReader.readBatch(from: file) {
                do {
                    ...
                    if uploadStatus.needsRetry {
                        AT.logger.debug("   → (\(self.featureName)) not delivered, will be retransmitted: \(uploadStatus.userDebugDescription)")
                        self.delay.increase()
                        self.scheduleNextCycle()
                        return
                    }

                    AT.logger.debug("   → (\(self.featureName)) accepted, won't be retransmitted: \(uploadStatus.userDebugDescription)")
                    if files.isEmpty {
                        self.delay.reset()
                    }
