...
extension DataUploadStatus {
    ...
    init(httpResponse: HTTPURLResponse, atRequestID: String?, attempt: UInt) {
        let statusCode = HTTPResponseStatusCode(rawValue: httpResponse.statusCode) ?? .unexpected

        self.init(
            needsRetry: statusCode.needsRetry,
            responseCode: httpResponse.statusCode,
            userDebugDescription: "[response code: \(httpResponse.statusCode) (\(statusCode)), request ID: \(atRequestID ?? "(???)")",
    ...
    init(networkError: Error, attempt: UInt) {
        self.init(
            ...
            responseCode: nil,
            userDebugDescription: "[error: \(ATError(error: networkError).message)]", // e.g. "[error: A data connection is not currently allowed]"
