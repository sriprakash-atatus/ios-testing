...
// ATCHG: Atatus SDK migration - renamed module imports `ddCore` -> `AtatusCore`; renamed `dd*` members
// to `at*`; rebranded the licence header.
...
class DataUploadStatusTests: XCTestCase {
    ...
    func testWhenUploadFinishesWithResponse_andStatusCodeNeedsNoRetry_itSetsNeedsRetryFlagToFalse() {
        statusCodesExpectingNoRetry.forEach { statusCode, _ in
            let status = DataUploadStatus(httpResponse: .mockResponseWith(statusCode: statusCode), atRequestID: .mockAny(), attempt: 0)
    ...
    func testWhenUploadFinishesWithResponse_andRequestIDIsAvailable_itCreatesUserDebugDescription() {
        expectedStatusCodes.forEach { statusCode, message in
            ...
            let status = DataUploadStatus(httpResponse: .mockResponseWith(statusCode: statusCode), atRequestID: requestID, attempt: 0)
    ...
    func testWhenUploadFinishesWithResponse_andRequestIDIsNotAvailable_itCreatesUserDebugDescription() {
        expectedStatusCodes.forEach { statusCode, message in
            let status = DataUploadStatus(httpResponse: .mockResponseWith(statusCode: statusCode), atRequestID: nil, attempt: 0)
    ...
    func testWhenUploadFinishesWithResponse_andStatusCodeIs401_itCreatesError() {
        let status = DataUploadStatus(httpResponse: .mockResponseWith(statusCode: 401), atRequestID: nil, attempt: 0)
    ...
    func testWhenUploadFinishesWithResponse_itSetsResponseCode() {
        ...
        let status = DataUploadStatus(httpResponse: .mockResponseWith(statusCode: randomCode), atRequestID: nil, attempt: 0)
