...
class InternalLoggerTests: XCTestCase {
    ...
    func testConcurrentLoggerReplacementDoesNotCrash() {
        // Ensures AT.logger can be safely replaced while being accessed from multiple threads
        let expectation = self.expectation(description: "Concurrent operations complete")
        expectation.expectedFulfillmentCount = 10

        // Simulate multiple threads reading AT.logger
        for threadId in 0..<8 {
            DispatchQueue.global(qos: .userInitiated).async {
                for i in 0..<1_000 {
                    AT.logger.debug("Thread \(threadId) message \(i)")
                    AT.logger.error("Thread \(threadId) error \(i)")
                }
                expectation.fulfill()
            }
        }

        // Simulate threads replacing AT.logger (like during SDK initialization)
        for _ in 0..<2 {
            DispatchQueue.global(qos: .userInitiated).async {
                for _ in 0..<200 {
