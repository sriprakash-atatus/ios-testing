...
class LoggerTests: XCTestCase {
        ...
        """, usingCustomKeyComparators: [
            "atatusTags": { lhs, rhs, file, line in
                guard let lhs = lhs as? String,
                    ...
                    XCTFail("atTags values are not strings.", file: file, line: line)
                    return
                }
    ...
    func testSendingTagsWithVariant() throws {
        ...
        let logMatchers = try core.waitAndReturnLogMatchers()
        logMatchers[0].assertTags(equal: ["tag1", "env:tests", "version:1.2.3", "appName:integration", "service:default-service-name", "sdk_version:3.2.1"])
        logMatchers[1].assertTags(equal: ["tag1", "tag2:abcd", "env:tests", "version:1.2.3", "appName:integration", "service:default-service-name", "sdk_version:3.2.1"])
        logMatchers[2].assertTags(equal: ["env:tests", "version:1.2.3", "appName:integration", "service:default-service-name", "sdk_version:3.2.1"])
    }
