...
class ATRUMConfigurationTests: XCTestCase {
    ...
    func testSetDDRUMURLSessionTrackingWithFirstPartyHosts() {
        ...
        ATAssertReflectionEqual(swift.urlSessionTracking, RUM.Configuration.URLSessionTracking())
        ...
        tracking.setFirstPartyHostsTracing(.init(hostsWithHeaderTypes: ["foo.com": [.b3, .atatus]]))
        objc.setURLSessionTracking(tracking)
        ...
        tracking.setFirstPartyHostsTracing(.init(hostsWithHeaderTypes: ["foo.com": [.b3, .atatus]], sampleRate: 99))
        objc.setURLSessionTracking(tracking)
    ...
    func testSetDDRUMURLSessionTrackingWithTrackResourceHeaders() {
        ...
        objc.setURLSessionTracking(tracking)
        ATAssertReflectionEqual(swift.urlSessionTracking, .init(trackResourceHeaders: .disabled))

        tracking.setTrackResourceHeaders(.defaults)
        objc.setURLSessionTracking(tracking)
        ATAssertReflectionEqual(swift.urlSessionTracking, .init(trackResourceHeaders: .defaults))

        tracking.setTrackResourceHeaders(.custom([.defaults]))
        objc.setURLSessionTracking(tracking)
        ATAssertReflectionEqual(swift.urlSessionTracking, .init(trackResourceHeaders: .custom([.defaults])))

        tracking.setTrackResourceHeaders(.custom([.matchHeaders(["x-request-id", "x-trace-id"])]))
        objc.setURLSessionTracking(tracking)
        ATAssertReflectionEqual(swift.urlSessionTracking, .init(trackResourceHeaders: .custom([.matchHeaders(["x-request-id", "x-trace-id"])])))

        tracking.setTrackResourceHeaders(.custom([.defaults, .matchHeaders(["x-request-id"])]))
        objc.setURLSessionTracking(tracking)
        ATAssertReflectionEqual(swift.urlSessionTracking, .init(trackResourceHeaders: .custom([.defaults, .matchHeaders(["x-request-id"])])))
    }
    ...
    func testEventMappers() {
        ...
        objc.setViewEventMapper { objcViewEvent in
            ATAssertReflectionEqual(objcViewEvent.swiftModel, swiftViewEvent)
            objcViewEvent.view.url = "redacted view.url"
            return objcViewEvent
        }

        objc.setResourceEventMapper { objcResourceEvent in
            ATAssertReflectionEqual(objcResourceEvent.swiftModel, swiftResourceEvent)
        ...
        objc.setActionEventMapper { objcActionEvent in
            ATAssertReflectionEqual(objcActionEvent.swiftModel, swiftActionEvent)
        ...
        objc.setErrorEventMapper { objcErrorEvent in
            ATAssertReflectionEqual(objcErrorEvent.swiftModel, swiftErrorEvent)
        ...
        objc.setLongTaskEventMapper { objcLongTaskEvent in
            ATAssertReflectionEqual(objcLongTaskEvent.swiftModel, swiftLongTaskEvent)
